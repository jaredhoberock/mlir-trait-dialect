...
// "fresh" cannot alias a variable in hand. An equality claim seals its endpoints
// against the walk, so a variable spelled only inside an endpoint is invisible to
// a naive scan -- and the mint would then hand a sibling generic the very id the
// endpoint holds, conflating two variables. The impl's self claim below spells
// `!trait.poly<0>` in the first argument and, sealed inside the equality endpoint
// of the second, `!trait.infer<0>`. The derive binds the poly to `f64` and the
...
trait.trait @Trait[!trait.poly<0>, !trait.poly<1>] {}
trait.trait @Need[!trait.poly<0>] {}

trait.impl @I for @Trait[!P, !trait.claim<!I = i32>] where [@Need[!P]] {}

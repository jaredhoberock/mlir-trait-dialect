...
// the failure is attributed to the op that carries it -- a loud diagnostic ahead
// of the legalization failure the leftover projection triggers downstream --
// rather than surfacing only as that opaque failure.
...
func.func @main() -> !trait.proj<@T[i64], "A"> {
  // expected-warning @below {{unresolved projection '!trait.proj<@T[i64], "A">' after instantiate-monomorphs}}

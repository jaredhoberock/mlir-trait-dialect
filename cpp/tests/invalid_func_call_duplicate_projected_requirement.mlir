...
// Duplicate proof bindings are accepted only when they are coherent. This case
// reaches the same projected obligation two ways -- through @T2's trait
// requirement, discharged by @T1_i64_a, and through the callee's own claim
// parameter, carried by @T1_i64_b -- so the substitution the call builds binds
// @T1[i64] twice, to two different proofs, and is refused.

// CHECK: error: 'trait.func.call' op inconsistent proof mapping: '!trait.claim<@T1[i64]>' is already bound to '!trait.claim<@T1[i64] by @T1_i64_a>', but attempted to bind '!trait.claim<@T1[i64] by @T1_i64_b>'
// CHECK-NEXT: trait.func.call @f(%x, %t2, %t1_projected)
